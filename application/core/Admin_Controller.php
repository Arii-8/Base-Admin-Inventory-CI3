	...
	private function createTree($parent, $menu,$parent_id,$path_active_name){
	 	...
	 	$html = "";
	   	if (isset($menu['parents'][$parent])) {
	      	if($parent == 1){
	      		$html .= '<ul class="nav nav-pills nav-sidebar flex-column nav-compact nav-flat" data-widget="treeview" role="menu" data-accordion="true"> ';
	      	}else{
	       	...
	       	foreach ($menu['parents'][$parent] as $itemId) { 
	          	...
	          	if(!isset($menu['parents'][$itemId])) { 
	          		$class = ($path_active_name == strtolower($urlText) )?"class ='nav-item'":"class ='nav-item'";
	          		$active = ($path_active_name == strtolower($urlText))?"active":'';
	             	$html .= '<li '.$class.'>
     				<a href="'.$url.'"class="nav-link '.$active.'"><i class="nav-icon fas '.$icon.'"></i> <p>'.$name.'</p></a>
     				</li>'; 
	          	}else {
	          		$class = ($id==$parent_id)?"menu-open":'';
	          		$active = ($id==$parent_id)?"active":'';
		         	$html .='<li class="nav-item '.$class.'"><a href="#" class="nav-link '.$active.'">
